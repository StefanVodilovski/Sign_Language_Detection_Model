...
def extract_keypoints(results):
    pose = np.array([[res.x, res.y, res.z,res.visibility] for res in results.pose_landmarks.landmark]).flatten() if results. \
        ...
        for action in actions:
            for sequence in range(no_sequences):
                for frame_number in range(sequence_length):
                    ...
                    if frame_number == 0:
                        cv2.putText(image,'STARTING COLL',(120,200),cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0),4,cv2.LINE_AA)
                        cv2.putText(image, f'COLLECTING FRAMES FOR {action} VIDEO NUMBER {sequence}', (15, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1,
                                    cv2.LINE_AA)
                        cv2.waitKey(500)
                    else:
